...
function createEditor(onSave, initial = {}) {
  ...
  const { InventoryPurchasePriceEditor } = harness({}, jsx).functions(["InventoryPurchasePriceEditor"], {
    ...runtime.hooks, ft: (value) => `${value} Ft`,
  }, priceFile);
...
test("purchase price editor: read and edit modes are entirely hidden from print output", () => {
  ...
  assert.match(run.text(), /120000 Ft/); assert.equal(run.text(true), "");
...
function createWarehouse(priceState = {}) {
  const runtime = hookRuntime(), stockCalls = [];
  function Editor() {}
  const { WarehousePanel } = harness({}, jsx).functions(["WarehousePanel"], {
    ...runtime.hooks, inventoryPriceKey, InventoryPurchasePriceEditor: Editor,
    useInventoryPurchasePrices: () => ({ prices: new Map(), loading: false, error: "", retry: noop, save: noop, ...priceState }),
    Shell: "main", Back: "back", ClimateProductManager: "manager", Layout: "layout", Main: "section", Side: "aside",
    Card: "card", StockBadge: "badge", Gradient: "gradient", InfoRow: "row", Field: "label", statusPillClass: String,
  ...
  const props = { workspaceId: "workspace-a", products, materialInventory: materials, stockOf: () => 10, reservedForProduct: () => 2,
    materialReserved: () => 3, addStock: (...args) => stockCalls.push(["climate", ...args]), addMaterialStock: (...args) => stockCalls.push(["material", ...args]) };
  let tree;
  const run = { stockCalls, render() { tree = runtime.render(() => WarehousePanel(props)); },
...
test("warehouse: all rows above ten remain on one page, with exact stock handlers and purchase-price targets", () => {
  ...
  stockButtons[23].props.onClick(); stockButtons[46].props.onClick();
  assert.deepEqual(run.stockCalls, [["climate", "ac-23", 3], ["material", "Szerelési anyag 22", 3]]);
  assert.equal(run.editors()[23].props.item.itemKey, "ac-23");
  assert.equal(run.editors()[46].props.item.itemKey, "Szerelési anyag 22");
});
