...
-- Diagnostic: jobs.legacy_source_key is required by save_appointment_with_job_mirror.
select
  'jobs.legacy_source_key exists before fix' as check_name,
  exists (
    select 1
...
-- Verification: required schema is present after the fix.
select *
from (
  values
    (
