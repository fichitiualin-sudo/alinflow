      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-7">
        {days.map(({ d, current }) => {
          ...
          return (
            <div
              key={dayIso}
              onClick={() => selectable ? onSelect?.(dayIso) : current ? onCreate?.(dayIso) : undefined}
              className={`min-h-[96px] rounded-3xl border p-3 md:min-h-[125px] ${isSelected ? "border-emerald-300 bg-emerald-400/20" : current ? "border-white/10 bg-slate-900/80" : "border-white/5 bg-slate-950/40 opacity-40"} ${selectable || (current && onCreate) ? "cursor-pointer hover:ring-2 hover:ring-emerald-300/50" : ""}`}
            >
              ...
              {current && onCreate && !selectable ? (
                <button
                  type="button"
                  onClick={(event) => { event.stopPropagation(); onCreate(dayIso); }}
                  className="mt-2 w-full rounded-2xl border border-cyan-300/20 bg-cyan-300/10 px-3 py-2 text-xs font-black text-cyan-100 transition hover:border-cyan-300/50 hover:bg-cyan-300/20"
                >
                  + Időpont
                </button>
              ) : null}
